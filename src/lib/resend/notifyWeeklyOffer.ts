...
// du lancement de l'offre hebdomadaire (4,50€/semaine) auprès des inscrits
...
export async function notifyWeeklyOffer(email: string, fullName: string | null) {
  ...
  await resend.emails.send({
    ...
    to: email,
    subject: "Nouveau sur Stageio : Premium à 4,50€ la semaine",
    html: `<p>Salut${fullName ? ` ${fullName}` : ""},</p>
<p>Tu as déjà utilisé tes swipes gratuits sur Stageio -- on vient d'ajouter une nouvelle façon de débloquer Premium, sans engagement mensuel : <strong>4,50€ pour une semaine</strong>.</p>
