-- Hard paywall dès le 2e swipe : le quota gratuit de découverte passe de
-- 4/semaine à 1/semaine. Miroir de FREE_WEEKLY_SWIPE_QUOTA côté app
-- (src/lib/subscription/quota.ts) -- les deux doivent toujours matcher.
...
begin
  ...
  if v_recent_browse_count >= 1 then
    raise exception 'SWIPE_QUOTA_REACHED'
      using errcode = 'P0001',
            hint = 'Quota hebdomadaire de swipes gratuits atteint (1/semaine).';
  end if;
